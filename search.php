        ...
        <div class="product-main">
          ...
          <div class="product-grid">
            ...
            if (isset($_POST['submit']) and !(empty($_POST['search'])) or isset($_POST['catag'])) {
              ...
              // Loop over each item in search term array
              $search_conditions = [];
              for ($search = 0; $search < sizeof($search_term_array); $search++) {
                // Add search conditions for both title and category
                $search_conditions[] = "products.product_title LIKE '%{$search_term_array[$search]}%' OR products.product_catag LIKE '%{$search_term_array[$search]}%'";
              }

              // Join all the conditions with 'AND' to handle multiple words
              $search_query .= implode(" AND ", $search_conditions);

              // Add ORDER BY, LIMIT, and OFFSET clauses
              $search_query .= " ORDER BY products.product_id DESC LIMIT {$offset}, {$limit}";
